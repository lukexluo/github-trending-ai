...
def infer_tags(name, description, lang):
    ...
    if "agent" in text:
        tags.append("agent")
    if any(k in text for k in ["ai", "llm", "gpt", "claude", "openai", "gemini", "kimi", "glm", "zhipu", "deepseek"]):
        tags.append("AI")
    if "mcp" in text:
        tags.append("MCP")
    if "codex" in text:
        tags.append("codex")
    if any(k in text for k in ["coding", "code", "programming", "developer", "gaming", "music", "video"]):
        tags.append("coding")
    if "memory" in text:
        tags.append("memory")
    if "skill" in text:
